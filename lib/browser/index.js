...
class Corrosion extends require("../rewrite") {
  constructor(config = defaultConfig) {
    ...
    this._url = new URL(
      this.config.url ||
        this.url.unwrap(this.window.location.href, {
    ...
    this.meta = {
      ...
      get base() {
        ...
        return corrosion.window.document.baseURI != corrosion.location.href
          ? new URL(corrosion.window.document.baseURI)
          : corrosion._url;
      },
      url: this._url,
    };
    ...
    if (!this.serviceWorker && this.window.document.currentScript)
      this.window.document.currentScript.remove();
  }
  get parent() {
  ...
  get$m(obj, key) {
    ...
    if (
      (obj == this.window && key == "location") ||
      (!this.serviceWorker && obj == this.window.document && key == "location")
    )
