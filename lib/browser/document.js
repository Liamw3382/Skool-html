function createDocumentRewriter(ctx) {
  return function rewriteDocument() {
    ...
    const cookie = Object.getOwnPropertyDescriptor(
      ctx.window.Document.prototype,
      "cookie"
    );
    const domain = Object.getOwnPropertyDescriptor(
      ctx.window.Document.prototype,
      "domain"
    );
    ...
    const baseURI = Object.getOwnPropertyDescriptor(
      ctx.window.Node.prototype,
      "baseURI"
    );
    const cookieEnabled = Object.getOwnPropertyDescriptor(
      ctx.window.Navigator.prototype,
      "cookieEnabled"
    );
    let spoofTitle = "";
    ...
    if (ctx.window.Document.prototype.write) {
      ctx.window.Document.prototype.write = new Proxy(
        ctx.window.Document.prototype.write,
        {
      ...
      Object.defineProperty(ctx.window.Document.prototype, "cookie", {
        get: new Proxy(cookie.get, {
          apply: (target, that, args) => {
            ...
            return ctx.config.cookie
              ? ctx.cookies.decode(cookies, ctx.meta)
              : "";
          },
        }),
        set: new Proxy(cookie.set, {
          apply: (target, that, [val]) => {
            return Reflect.apply(target, that, [
              ctx.config.cookie ? ctx.cookies.encode(val, ctx.meta) : "",
            ]);
          },
        }),
      });
    ...
    if (ctx.window.Document.prototype.writeln) {
      ctx.window.Document.prototype.writeln = new Proxy(
        ctx.window.Document.prototype.writeln,
        {
          apply: (target, that, args) => {
            if (args.length) args = [ctx.html.process(args.join(""), ctx.meta)];
            return Reflect.apply(target, that, args);
    ...
    if (ctx.window.Element.prototype.setAttribute) {
      ctx.window.Element.prototype.setAttribute = new Proxy(
        ctx.window.Element.prototype.setAttribute,
        {
          apply: (target, that, args) => {
            if (args[0] && args[1]) {
    ...
    if (ctx.window.Element.prototype.getAttribute) {
      ctx.window.Element.prototype.getAttribute = new Proxy(
        ctx.window.Element.prototype.getAttribute,
        {
          apply: (target, that, args) => {
            if (args[0] && that.hasAttribute(`corrosion-${args[0]}`))
              args[0] = `corrosion-${args[0]}`;
            return Reflect.apply(target, that, args);
          },
        }
      );
    }
    ctx.window.CSSStyleDeclaration.prototype.setProperty = new Proxy(
      ctx.window.CSSStyleDeclaration.prototype.setProperty,
      {
        apply: (target, that, args) => {
          if (args[1])
            args[1] = ctx.css.process(args[1], {
    ...
    ["innerHTML", "outerHTML"].forEach((html) => {
      const descriptor = Object.getOwnPropertyDescriptor(
        ctx.window.Element.prototype,
        html
      );
      Object.defineProperty(ctx.window.Element.prototype, html, {
        get: new Proxy(descriptor.get, {
          apply: (target, that, args) => {
            ...
            if (!body || (html == "innerHTML" && that.tagName == "SCRIPT"))
              return body;
        ...
        set: new Proxy(descriptor.set, {
          apply(target, that, [val]) {
            return Reflect.apply(target, that, [
              val ? ctx.html.process(val.toString(), ctx.meta) : val,
            ]);
          },
        }),
      });
    ...
    [
      ...
      Object.defineProperty(
        ctx.window.CSS2Properties
          ? ctx.window.CSS2Properties.prototype
          : ctx.window.CSSStyleDeclaration.prototype,
        key,
        {
          get() {
      ...
      set: new Proxy(domain.set, {
        apply: (target, that, [val]) => {
          if (
            !val
              .toString()
              .endsWith(ctx.location.hostname.split(".").slice(-2).join("."))
          )
            return Reflect.apply(target, that, [""]);
    ...
    Object.defineProperty(ctx.window.Node.prototype, "baseURI", {
      get: new Proxy(baseURI.get, {
        apply: (target, that, args) => {
          ...
          return val.startsWith(ctx.meta.origin)
            ? ctx.url.unwrap(val, ctx.meta)
            : val;
        },
      }),
    });
