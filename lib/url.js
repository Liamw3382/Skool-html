...
class URLWrapper {
  ...
  wrap(val, config = {}) {
      ...
      try {
        if (
          !["http:", "https:", "ws:", "wss:"].includes(
            new URL(val, config.base).protocol
          )
        )
          return val;
      } catch (e) {
    ...
    return (
      ...
      this.prefix +
      flags +
      this.codec.encode(config.base ? new URL(val, config.base) : val) +
      "/"
    );
  }
  unwrap(val, config = {}) {
    if (!val || this.regex.test(val)) return val;
    let processed = val.slice(
      (config.origin || "").length + this.prefix.length
    );
    ...
    return config.flags
      ? {
          value:
            this.codec.decode(url || "") +
            (config.leftovers && leftovers ? leftovers : ""),
          flags,
        }
      : this.codec.decode(url || "") +
          (config.leftovers && leftovers ? leftovers : "");
