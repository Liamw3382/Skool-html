...
class JSRewriter {
    ...
    this.map = [
      {
        ...
        handler: (node, parent) => {
          ...
          if (rewrite) {
            ...
            const args = [node.object, node.property];
            ...
            if (parent.type == "UpdateExpression") {
              ...
              args.push(
                ...
                this.createLiteral(parent.operator)
              );
            }
            Object.assign(
              nodeToRewrite,
      ...
      },
      {
        ...
        handler: (node, parent) => {
          ...
          let nodeToRewrite = node;
          const args = [this.createIdentifier(node.name, true)];
      ...
      },
      {
        ...
        handler: (node, parent, url) => {
          ...
          node.source = this.createLiteral(
            ctx.url.wrap(node.source.value, { base: url })
          );
        },
      },
      {
        ...
        handler: (node, parent) => {
          node.source = this.createCallExpression(
            this.createMemberExpression(
              this.createMemberExpression(
                this.createIdentifier("$corrosion"),
                this.createIdentifier("url")
              ),
              this.createIdentifier("wrap")
            ),
            [
              node.source,
              this.createMemberExpression(
                this.createIdentifier("$corrosion"),
                this.createIdentifier("meta")
              ),
            ]
          );
        },
  ...
  process(source, url) {
    try {
      ...
      return (
        (url ? this.createHead(url) : "") +
        generate(ast, this.generationOptions)
      );
    } catch (e) {
      return source;
    }
  ...
  createHead(url) {
    return `
        if (!self.$corrosion && self.importScripts) {
            ...
            self.$corrosion = new Corrosion({ url: '${url}', codec: '${
      this.ctx.config.codec || "plain"
    }', serviceWorker: true,  window: self, prefix: '${
      this.ctx.prefix || "/service/"
    }', ws: ${this.ctx.config.ws || true}, cookies: ${
      this.ctx.config.cookies || false
    }, title: '${this.ctx.config.title}', }); $corrosion.init();
        };\n`;
  }
  iterate(ast, handler) {
