    ...
    iterate(ast, (node) => {
      ...
      switch (node.tagName) {
        case "STYLE":
          ...
          break;
        case "TITLE":
          if (node.textContent && this.ctx.config.title)
    ...
    return [
      ...
      },
      {
        ...
        childNodes: [
          {
            nodeName: "#text",
            value: `window.$corrosion = new Corrosion({ window, codec: '${
              this.ctx.config.codec || "plain"
            }',  prefix: '${this.ctx.prefix}', ws: ${
              this.ctx.config.ws
            }, cookie: ${this.ctx.config.cookie}, title: ${
              typeof this.ctx.config.title == "boolean"
                ? this.ctx.config.title
                : "'" + this.ctx.config.title + "'"
            }, }); $corrosion.init(); document.currentScript.remove();`,
          },
        ],
        attrs: [],
      },
