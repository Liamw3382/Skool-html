      ...
      .on("end", () => {
        ...
        const query =
          clientRequest.method == "POST"
            ...
            : new URLSearchParams(clientRequest.url.split("?")[1] || "");
        if (!query.has("url")) return clientResponse.end();
        const url = query.get("url");
        if (
          /https?:\/\/([a-zA-Z0-9\-\_])|([a-zA-Z0-9\-\_])\.([a-zA-Z])/.test(url)
        ) {
          clientResponse.writeHead(301, {
            Location: ctx.url.wrap(
          ...
          clientResponse.writeHead(301, {
            Location: ctx.url.wrap("https://www.google.com/search?q=" + url),
          });
          clientResponse.end();
        }
      });
