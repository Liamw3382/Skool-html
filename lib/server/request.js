...
function createRequestProxy(ctx) {
  return async function onRequest(clientRequest, clientResponse) {
    try {
      if (new RegExp(`^${ctx.prefix}gateway/?`).test(clientRequest.url))
        return ctx.gateway(clientRequest, clientResponse);
      ...
      const requestContext = {
        ...
        flags: urlData.flags,
        origin:
          (clientRequest.socket.encrypted || ctx.config.forceHttps
            ? "https://"
            : "http://") + clientRequest.headers.host,
        body: await getChunks(clientRequest),
        headers: { ...clientRequest.headers },
        method: clientRequest.method,
        rewrite: ctx,
        agent: new (urlData.value.protocol == "https:" || ctx.config.forceHttps
          ? https
          : http
        ).Agent({
      ...
      for (let i in ctx.config.requestMiddleware)
        ctx.config.requestMiddleware[i](requestContext);
      if (clientResponse.writableEnded) return;
      (requestContext.url.protocol == "https:" || ctx.config.forceHttps
        ? https
        : http
      )
        .request(
          ...
          async (remoteResponse) => {
            ...
            for (let i in ctx.config.responseMiddleware)
              ctx.config.responseMiddleware[i](responseContext);
            if (clientResponse.writableEnded) return;
