...
class Corrosion extends require("../rewrite") {
  constructor(config = defaultConfig) {
    ...
    if (this.config.standardMiddleware) {
      this.config.requestMiddleware.unshift(middleware.requestHeaders);
  ...
  bundleScripts() {
    webpack(
      ...
      (err) => console.log(err || "Bundled scripts")
    );
  }
  get script() {
    return fs.existsSync(path.join(__dirname, "bundle.js"))
      ? fs.readFileSync(path.join(__dirname, "bundle.js"))
      : "Client script is still compiling or has crashed.";
  }
}
