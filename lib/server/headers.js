function requestHeaders(ctx) {
  if (ctx.headers.cookie && ctx.rewrite.config.cookie)
    ctx.headers.cookie = ctx.rewrite.cookies.decode(ctx.headers.cookie, {
      url: ctx.url,
    });
  ...
  if (ctx.headers.referer) {
    try {
      ctx.headers.referer = new URL(
        ctx.rewrite.url.unwrap(ctx.headers.referer, { origin: ctx.origin })
      ).href;
    } catch (err) {
...
function responseHeaders(ctx) {
  if (ctx.headers.location)
    ctx.headers.location = ctx.rewrite.url.wrap(ctx.headers.location, {
      base: ctx.url,
      origin: ctx.origin,
    });
  if (ctx.headers["set-cookie"])
    ctx.headers["set-cookie"] = ctx.rewrite.cookies.encode(
      ctx.headers["set-cookie"],
      { domain: ctx.clientRequest.headers.host, url: ctx.url }
    );
  [
    "content-length",
