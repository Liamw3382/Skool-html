...
class CSSRewriter {
  ...
  process(source, config = {}) {
    ...
    const urls = csstree.findAll(ast, (node) => node.type == "Url");
    const imports = csstree.findAll(
      ast,
      (node) =>
        node.type == "Atrule" &&
        node.name == "import" &&
        ...
        node.prelude.children.head.data.type == "String"
    );
    urls.forEach(({ value }) => {
      switch (value.type) {
        case "String":
          ...
          value.value =
            quote +
            this.ctx.url.wrap(value.value.slice(1).slice(0, -1), config) +
            quote;
          break;
        case "Raw":
    ...
    imports.forEach(({ prelude }) => {
      ...
      data.value =
        quote +
        this.ctx.url.wrap(data.value.slice(1).slice(0, -1), config) +
        quote;
    });
  ...
  source(processed, config = {}) {
    ...
    urls.forEach(({ value }) => {
      switch (value.type) {
        case "String":
          ...
          value.value =
            quote +
            this.ctx.url.unwrap(value.value.slice(1).slice(0, -1), config) +
            quote;
          break;
        case "Raw":
    ...
    imports.forEach(({ prelude }) => {
      ...
      data.value =
        quote +
        this.ctx.url.unwrap(data.value.slice(1).slice(0, -1), config) +
        quote;
    });
    return csstree.generate(ast);
  }
